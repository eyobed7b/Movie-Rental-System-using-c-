...
return 0;
             ...
             void menu::f_page()
 {
	...
	gotoxy(10,15);
    cout<<"Loading ";
system("speaker.vbs \" K E D SOFTAWRE STUDIO\"");
	for(int i=0; i<=9; i++)
	{ 
		gotoxy(i+20,15);
    	cout<<"**";
...
menu_f(); 
	               ...
	               void menu::filter(int i)
	               {
				  		...
				  		gotoxy(36,17);cin>>f.data[i].id_no;
				  			...
				  			gotoxy(36,11);cin>>f.data[i].f_name;   	 goto filter; break;
							  } }
							  for(int k=0; f.data[i].f_name[k]!=0;k++)
							   if (f.data[i].f_name[k]>='0'&& f.data[i].f_name[k]<='9')
							        {
    ...
    {
    	if(strcmp(id_no,f.data[i].id_no)==0)
    	{
    		searched_data(i);
    		break;
		}
	 ...
	 {  a:
	 	   char delet;
	 	gotoxy(20,23); cout<<"Id Found. Are sure ? (y/n)"; delet=getch();
	 	  if(delet=='N'||delet=='n')  
		   {
		   remove("temp.txt");
		   menu_f();
		   }
		   else  if(delet=='Y'||delet=='y')  
		   {
...
{    
	 ...
	 gotoxy(6,7);cout<<"  1. Full  NAME "<<endl<<"\n"<<" \t"<<"2. ID NUMBER"<<endl<<"\n"<<"\t"<<"3. MOVIE NAME "<<endl<<"\n"
	                <<"\t"<<"4. MOVIE NO "<<endl<<"\n"<<"\t"<<"5. PHONE NUMBER";
     ::forall=getch();
	...
	switch(::forall)
{
	...
	menu_f();
        ...
        {       char format;  
			   ...
			   {   top:
			   	    system("cls"); system("type null.txt"); display_time();
			   	    gotoxy(9,10);cout<<"1.custemers Data     2.trailers Data      3.income Data";
			   	    gotoxy(30,12);cout<<"4.All Data";
                format=getch();
                if(format=='1')
			   	{	
				   remove("student.txt");
		    ...
		    if(::forall=='1')  
		     ...
		     {   string m_name; bool found=false;
                ...
                { 
                 ...
                 
				 {
                   ...
                   for(int i=0; i<(::trail);i++)
                   {
                     ...
                     gotoxy(34,y);
                       if   ( ((f.trails[i].day)-(itm->tm_mday)<=0)&&((f.trails[i].month)-(1+itm->tm_mon)==0)&&((f.trails[i].year)-(1900+itm->tm_year)==0) ||((f.trails[i].month)-(1+itm->tm_mon)<0)
					          &&((f.trails[i].year)-(1900+itm->tm_year)==0)||((f.trails[i].year)-(1900+itm->tm_year)<0)||((f.trails[i].year)-(1900+itm->tm_year)<=0)
					          && ((f.trails[i].month)-(1+itm->tm_mon)<=0) &&((f.trails[i].day)-(itm->tm_mday)<=0))
					             {
					             	  delet_trail(f.trails[i].name); ::trail--;  
					             	  
								 }
                      if   ((f.trails[i].year)-(1900+itm->tm_year)>0&&(f.trails[i].month)-(1+itm->tm_mon)<=0)
                            {        if((f.trails[i].month)-(1+itm->tm_mon)==0)
							     cout<<(f.trails[i].year)-(1900+itm->tm_year)<<" Year"<<setw(2);
							      else if((f.trails[i].month)-(1+itm->tm_mon)<=0)
                                 cout<<12-(1+itm->tm_mon)+(f.trails[i].month)<<" Month"<<setw(2);
                                 if((f.trails[i].day)>(itm->tm_mday  ))
				...
				{ 
					...
					system("cls"); gotoxy(10,10); cout<<"NO DATA";::forall=getch();
				}
				...
				{
					ofstream mo;
					mo.open("trailer.txt",ios::app);
				
						mo<<f.trails[i].name<<"\t"<<f.trails[i].year<<"\t"<<f.trails[i].month<<"\t"<<f.trails[i].day<<endl;
			     ...
			     gotoxy(22,6);	cin>>f.trails[k].name;               gotoxy(16,8);cin>>f.trails[k].year;   
					...
					void menu::trailer_view()
					{
						...
						void menu::write_total_income(char id_no[10] )
						{
							...
							for(int i=0; i<(::users)+1;i++)
							{
								...
								write<<f.data[i].movie_l<<"\t"  <<f.data[i].movie_t<<"\t"<<f.data[i].m_name
                                    ...
                                    <<" \t"<< f.time[i].t_hour<<"\t"<< f.time[i].t_min<<"\t"<<f.time[i].t_sec<<"\t"<<costf(i)<<"\t"<<tax(i)<<"\t"<<total(i)<< endl;
								  break;
								  }
								}
							}
					 ...
					 int menu::read_total_income()
					 {
					 	...
					 	int m=0;
					 	while(read>>f.income[m].movie_l>>f.income[m].movie_t>>f.income[m].m_name>>f.income[m].m_no>>f.income[m].quantity>>
            	             f.income[m].t_year>> f.income[m].t_month>> f.income[m].t_day >>f.income[m].t_hour>> f.income[m].t_min>>f.income[m].t_sec>>f.income[m].cost
							 >>f.income[m].tax>>f.income[m].total)
							{
								m++;
								  }	  
					  ...
					  {
					  	      float rev=0, total_tax=0;  bool found=false;
					  		 ::income=read_total_income();
					  		 system("cls");  set_console_size(90,35);
	   ...
	   gotoxy(35,0);cout<<"| Movie"; gotoxy(35,1);cout<<"| No";gotoxy(42,0);cout<<"| Quan"; gotoxy(42,1); cout<<"| tity";
        gotoxy(48,0); cout<<"|   Date";gotoxy(48,1);cout<<"|";
    ...
    int j=3;
     ...
     {     
	              found=true;
	           gotoxy(0,j);  cout<<f.income[i].movie_l<<" "<<f.income[i].movie_t; gotoxy(22,j); 
		 	cout<<"|"<<f.income[i].m_name; gotoxy(35,j); cout<<"| "<<f.income[i].m_no;  gotoxy(42,j);    cout<<"| "<<f.income[i].quantity;
		 	  gotoxy(48,j) ;cout<<"| "<< f.income[i].t_year<<"/"<< f.income[i].t_month<< "/"<<f.income[i].t_day; gotoxy(59,j);
            cout<<"| "<<f.income[i].t_hour<<":"<< f.income[i].t_min<<":"<<f.income[i].t_sec; gotoxy(70,j); cout<<"|"<<f.income[i].cost; gotoxy(76,j);cout<<"    ";
            gotoxy(76,j); cout<<"| "<<f.income[i].tax; gotoxy(81,j); cout<<"      "; gotoxy(81,j); cout<<"| "<<f.income[i].total; gotoxy(86,j);cout<<"    ";
          j+=2;
	  ...
	  for(int i=0; i<(::income);i++)
	  {
	  	rev+=f.income[i].cost; total_tax+=f.income[i].tax;
	  }
	  gotoxy(3,j); cout<<"Revenue = "<<rev; gotoxy(42,j);cout<<"| "<<"            Total Tax = "<<total_tax;
	  gotoxy(0,j+1);	cout<<"------------------------------------------------------------------------------------------------------------------------";
	  rev=0; total_tax=0;
    ...
    if(!found)
          {
          	system("cls"); system("type null.txt"); gotoxy(10,10);cout<<"NO DATA";
